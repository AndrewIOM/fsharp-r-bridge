...
let environmentTests =
    testList "REnvironment" [
        ...
        testCase "createEmpty environments are unique" <| fun _ ->
        ...
        testCase "ofSExp recognises environments" <| fun _ ->
            let env = REnvironment.globalEnv engine.Value
            let sexp = { ptr = env.Pointer }
        ...
        testCase "ofSExp rejects non-environments" <| fun _ ->
            let str = Create.stringVector engine.Value [ "hello" ]
            match REnvironment.ofSExp engine.Value str with
            | None -> ()
            | Some _ -> failtest "ofSExp should return None for non-environment"

        testProperty "Lookup of bound symbol always returns Some" <| fun value ->
            let env = REnvironment.createEmpty engine.Value
            let eSym = NativeApi.install "env" engine.Value.Api
            NativeApi.defineVar
                eSym
                env.Pointer
                engine.Value.Api.globalEnv
                engine.Value.Api |> ignore
            ...
            let found = REnvironment.tryGetValue engine.Value env "x"
            match found with
            | None -> false
            | Some sexp ->
                SymbolicExpression.getType engine.Value sexp <> SymbolicExpression.Nil

        testProperty "Lookup of unknown symbol returns None" <| fun sym ->
            if System.String.IsNullOrEmpty sym || not (isSafeSymbolName sym) then true
            else
                printfn "Name = '%s'" sym
...
[<Tests>]
let evalTests =
    testList "Evaluation of expressions" [

        testCase "eval returns numeric type for 1+1" <| fun _ ->
            let glob = REnvironment.globalEnv engine.Value
            let sexp = Evaluate.eval "1+1" glob engine.Value
            let t = SymbolicExpression.getType engine.Value sexp
            Expect.equal t SymbolicExpression.RealVector "eval should not return a promise"
            let r = sexp |> Extract.extractFloatArray engine.Value
