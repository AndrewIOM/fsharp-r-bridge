...
module Attributes =
    ...
    let tryNames engine sexp : string option [] option =
        ...
        |> Option.bind
            (fun className ->
                ...
                | CharacterVector ->
                    ...
                    classNames |> Seq.tryHead |> Option.flatten
                | _ -> None)
...
module Dates =
    ...
    let isPosixDateTime engine sexp : bool =
        ...
        | RealVector ->
            ...
            List.contains (Some "POSIXct") classes
        | _ -> false
    ...
    let isDate engine sexp =
        ...
        | RealVector ->
            let classes = Classes.getClasses engine sexp
            classes = [Some "Date"] || List.contains (Some "Date") classes
        | _ -> false
...
module Factor =
    ...
    let isFactor engine sexp =
        if SymbolicExpression.getType engine sexp <> SymbolicExpression.IntegerVector then false
            ...
            | Some _ ->
                let classes = Classes.getClasses engine sexp
                List.contains (Some "factor") classes
