...
module SymbolicExpression =
    ...
    let getType (engine: RBridge.NativeApi.RunningEngine) (sexp: SymbolicExpression) : SexpType =
        if   engine.Api.typeof.isNull.Invoke sexp.ptr        <> 0 then Nil
        elif engine.Api.typeof.isSymbol.Invoke sexp.ptr      <> 0 then Symbol
        elif engine.Api.typeof.isEnvironment.Invoke sexp.ptr <> 0 then Environment
        elif engine.Api.typeof.isLogical.Invoke sexp.ptr     <> 0 then LogicalVector
        elif engine.Api.typeof.isInteger.Invoke sexp.ptr     <> 0 then IntegerVector
        elif engine.Api.typeof.isReal.Invoke sexp.ptr        <> 0 then RealVector
        elif engine.Api.typeof.isComplex.Invoke sexp.ptr     <> 0 then ComplexVector
        elif engine.Api.typeof.isString.Invoke sexp.ptr      <> 0 then CharacterVector
        elif engine.Api.typeof.isList.Invoke sexp.ptr        <> 0 then List
        elif engine.Api.typeof.isExpression.Invoke sexp.ptr  <> 0 then List
        else Any
    ...
    let getVectorElement (engine: NativeApi.RunningEngine) (sexp: SymbolicExpression) (index: int) : SymbolicExpression =
        match getType engine sexp with
        | LogicalVector ->
            let ptr = engine.Api.pointers.logicalPointer sexp.ptr
            let value = Marshal.ReadInt32(ptr, index * sizeof<int>)
            let out = engine.Api.allocVector.Invoke(typeAsInt LogicalVector, 1)
        ...
        | IntegerVector ->
            ...
            let value = Marshal.ReadInt32(ptr, index * sizeof<int>)
            let out = NativeApi.allocVector (typeAsInt IntegerVector) 1 engine
        ...
        | RealVector ->
            ...
            let bits = Marshal.ReadInt64(ptr, index * sizeof<int64>)
            let value = System.BitConverter.Int64BitsToDouble bits
            let out = engine.Api.allocVector.Invoke(typeAsInt RealVector, 1)
        ...
        | ComplexVector ->
            ...
            let out = engine.Api.allocVector.Invoke(typeAsInt ComplexVector, 1)
            let outPtr = engine.Api.pointers.realPointer out
        ...
        | CharacterVector ->
            let ptr = engine.Api.pointers.stringPointer sexp.ptr
        ...
        | RawVector ->
            ...
            let out = engine.Api.allocVector.Invoke(typeAsInt RawVector, 1)
        ...
        | s ->
            failwithf "Could not get vector element of type: %A" s
    ...
    let setVectorElement
        ...
        // Logical vector
        | LogicalVector ->
            let ptr = engine.Api.pointers.logicalPointer vector.ptr
            let vptr = engine.Api.pointers.logicalPointer value.ptr
            let v = Marshal.ReadInt32(vptr, 0)
            Marshal.WriteInt32(ptr, index * sizeof<int>, v)

        // Integer vector
        | IntegerVector ->
            let ptr = engine.Api.pointers.integerPointer vector.ptr
            let vptr = engine.Api.pointers.integerPointer value.ptr
            let v = Marshal.ReadInt32(vptr, 0)
            Marshal.WriteInt32(ptr, index * sizeof<int>, v)

        // Real vector
        | RealVector ->
            let ptr = engine.Api.pointers.realPointer vector.ptr
            let vptr = engine.Api.pointers.realPointer value.ptr
        ...
        | CharacterVector ->
            let ptr = engine.Api.pointers.stringPointer vector.ptr
            // value.ptr is a CHARSXP
            Marshal.WriteIntPtr(ptr, index * System.IntPtr.Size, value.ptr)

        // Raw vector
        | RawVector ->
            let ptr = engine.Api.pointers.charPointer vector.ptr
            let vptr = engine.Api.pointers.charPointer value.ptr
            let b = Marshal.ReadByte(vptr, 0)
            Marshal.WriteByte(ptr, index, b)

        // VECSXP / EXPRSXP
        | List ->
            let ptr = engine.Api.pointers.vectorPointer vector.ptr
            Marshal.WriteIntPtr(ptr, index * System.IntPtr.Size, value.ptr)

        | t ->
            failwithf "Cannot set element of type: %A" t
